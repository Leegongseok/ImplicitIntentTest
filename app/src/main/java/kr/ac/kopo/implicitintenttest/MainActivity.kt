...
class MainActivity : AppCompatActivity() {
    ...
    val btnListener= View.OnClickListener {
        ...
        when(it.id) {
            ...
            R.id.btn_home -> {
                uri = Uri.parse("http://www.kopo.ac.kr")
                intent = Intent(Intent.ACTION_VIEW, uri)
            }
            R.id.btn_map -> {
                uri = Uri.parse("http://maps.google.com?q=37.5290615,126.996542")
                intent = Intent(Intent.ACTION_VIEW, uri)
            }
            R.id.btn_search -> {
                intent = Intent(Intent.ACTION_WEB_SEARCH)
                intent.putExtra(SearchManager.QUERY,"한국인공지능소프트웨어")
            }
            R.id.btn_sms -> {
                intent = Intent(Intent.ACTION_SENDTO)
                intent.putExtra("sms_body","조금 있으면 나도 종강이야 우리한번 볼까")
